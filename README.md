...
| `fail-on-severities` | List\<String\> | No       | `NONE`                                                                                        | Fail the action if the given severities occur. In addition to the severities supported by SecHub we have the following special values:<ul><li>`NONE` - if you don't want to fail the action when severities found</li><li>`ALL` - if you want to fail on all severities</li></ul> |
